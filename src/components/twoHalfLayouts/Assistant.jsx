...
const Assistant = () => {
  ...
  return (
    <div className="relative flex flex-col h-screen bg-linear-to-r from-blue-400 to-blue-600">
      {/* 🧭 Header appears only when chats exist */}
      {hasChats && (
        ...
        <>
          <div className="flex-1 overflow-y-auto p-5 pb-28 space-y-1">
            {chatData.map((chat, index) => (
              <div
                key={index}
                className={`px-3 py-2 text-sm rounded-2xl max-w-[30%] mb-1
    wrap-break-words whitespace-pre-wrap break-all overflow-hidden leading-snug 
    ${
      chat.from === "user"
